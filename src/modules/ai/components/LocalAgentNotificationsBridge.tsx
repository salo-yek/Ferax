...
export function LocalAgentNotificationsBridge() {
  ...
  useEffect(() => {
    ...
    if (status === "awaiting-approval") {
      fire("attention", "Ferax needs your approval", "Approve a tool to continue");
    } else if (status === "error") {
      fire("error", "Ferax run failed", error ?? undefined);
    } else if (status === "idle" && isBusy(was)) {
      fire("finished", "Ferax finished", "Your task is ready");
    }
  }, [status, error]);
