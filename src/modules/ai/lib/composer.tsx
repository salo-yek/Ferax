...
type ComposerCtx = {
  ...
  /** Attach a file by absolute path â€” used by the file explorer's "Attach to Agent". */
...
export function AiComposerProvider({ children }: ProviderProps) {
  ...
  useEffect(() => {
    ...
    window.addEventListener("ferax:ai-attach-file", onAttach);
    return () => window.removeEventListener("ferax:ai-attach-file", onAttach);
  ...
  const submit = () => {
    ...
    // the prompt to the ferax.md scan template before sending.
    ...
    if (commandSource.startsWith("/") || commandSource.startsWith("#")) {
      ...
      if (outcome.kind === "send-prompt") {
        ...
        if (outcome.commandName) {
          commandMarker = `<ferax-command name="${outcome.commandName}" />`;
        }
      }
    }
