...
const INIT_PROMPT = `Scan this workspace and produce ferax.md at the workspace root with:
...
Use grep/glob/list_directory/read_file to explore. Cap ferax.md under 200 lines. Use write_file to create it (will go through normal approval).`;
...
export const ferax_cmd_re =
  /^<ferax-command\s+name="([a-z0-9-]+)"(?:\s+state="([a-z]+)")?\s*\/>(?:\n+|$)/;

export function wrapWithCommandMarker(prompt: string, name: string): string {
  return `<ferax-command name="${name}" />\n\n${prompt}`;
}
