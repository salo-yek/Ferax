...
const FERAX_MD_MAX_BYTES = 32 * 1024;
type MemoryCacheEntry = { content: string | null; mtime: number };
const projectMemoryCache = new Map<string, MemoryCacheEntry>();

async function readFeraxMd(workspaceRoot: string | null): Promise<string | null> {
  if (!workspaceRoot) return null;
  const path = `${workspaceRoot.replace(/\/$/, "")}/FERAX.md`;
  ...
  try {
    ...
    const content =
      r.content.length > FERAX_MD_MAX_BYTES
        ? r.content.slice(0, FERAX_MD_MAX_BYTES)
        : r.content;
...
export function createContextAwareTransport(deps: Deps) {
  const run = async (options: SendOptions) => {
    ...
    const projectMemory = await readFeraxMd(live.workspaceRoot);
    ...
    const result = await runAgentStream({
      ...
      autoExecute: deps.getAutoExecute?.() ?? false,
      autoEdit: deps.getAutoEdit?.() ?? false,
      projectMemory,
