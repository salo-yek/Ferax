  ...
  switch (provider) {
    ...
    case "openrouter": {
      ...
      built = createOpenAICompatible({
        ...
        apiKey: key,
        headers: {
          "HTTP-Referer": "https://nimbo.fun/saloyek/ferax",
          "X-Title": "Ferax",
        },
      })(resolvedModelId);
      break;
...
const AUTO_EXECUTE_PROMPT = `## AUTO-EXECUTE MODE — ACTIVE
bash_run and bash_background execute immediately without user confirmation. If any command fails (exit code ≠ 0), halt the chain and report the error to the user. Do NOT retry a command that previously failed with the same exit code — the system blocks cycles. Proceed with reads and mutations as normal.`;
...
function buildStableSystem(
  ...
  const memoryBlock =
    projectMemory && projectMemory.trim().length > 0
      ? `\n\n## PROJECT — FERAX.md\n${projectMemory.trim()}`
      : "";
  const autoBlock = autoExecute ? `\n\n${AUTO_EXECUTE_PROMPT}` : "";
  return `${base}${memoryBlock}${personaBlock}${customBlock}${autoBlock}`;
}
