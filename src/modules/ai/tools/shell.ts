...
export function buildShellTools(ctx: ToolContext, opts: ShellToolOptions = { autoExecute: false }) {
  const { autoExecute } = opts;
    ...
    bash_run: tool({
      ...
      execute: async ({ command, timeout_secs }) => {
        ...
        try {
          ...
          // When auto-executing, run the guard
          if (autoExecute) {
            const guard = checkAndRecord(sid, command, cmdResult);
            if (guard.kind === "cycle") {
              return {
                error: `⛔ AUTO-EXECUTE CYCLE DETECTED: Command "${command}" previously failed with exit code ${guard.lastResult.exit_code}. Halted.`,
                command,
                stdout: r.stdout,
                stderr: r.stderr,
                exit_code: r.exit_code,
            ...
            if (guard.kind === "failure") {
              return {
                error: `⛔ AUTO-EXECUTE FAILURE: Command "${command}" exited with code ${r.exit_code}. Auto-execution halted.`,
                command,
                stdout: r.stdout,
                stderr: r.stderr,
                exit_code: r.exit_code,
