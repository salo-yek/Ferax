...
export function buildFsTools(ctx: ToolContext, opts: FsToolOptions = {}) {
  const { autoEdit } = opts;
    ...
    read_file: tool({
      ...
      execute: async ({ path, offset, limit }) => {
        ...
        try {
          ...
          // Cache hit on full read — skip re-emitting the content
          if (isFullRead && prior && prior.hash === hash) {
            return {
              path: abs,
              unchanged: true,
              size: r.size,
              total_lines: r.content.split("\n").length,
            };
          }
          if (isFullRead) {
            ctx.readCache.set(abs, { size: r.content.length, hash });
          }

          const lines = r.content.split("\n");
          const start = offset ?? 0;
          const end = limit ? start + limit : Math.min(lines.length, READ_LINE_CAP);
