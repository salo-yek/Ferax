...
function getRecycler(): HTMLDivElement {
  ...
  el.setAttribute("data-ferax-recycler", "");
  el.style.cssText =
...
function createSlot(): Slot {
  ...
  host.setAttribute("data-ferax-slot", String(slots.length));
...
function bindSlot(slot: Slot, p: AcquireParams): void {
  ...
  if (p.snapshot) {
    try {
      ...
      console.warn("[ferax] snapshot replay failed:", e);
    }
  }
  if (p.altScreen) {
...
function serializeSlot(slot: Slot): SerializeOutput {
  ...
  try {
    ...
    console.warn("[ferax] serialize failed:", e);
  }
  return {
    snapshot,
    cols: slot.term.cols,
...
function attachWebgl(slot: Slot): void {
  ...
  try {
    ...
    console.warn("[ferax-webgl] unavailable:", e);
  }
}
...
function disposeSlotWebgl(slot: Slot): void {
  ...
  try {
    ...
    console.warn("[ferax-webgl] dispose failed:", e);
  }
  try {
    const r = (
