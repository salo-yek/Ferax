...
function distroCommand(key: DistroKey, version: string): string {
  switch (key) {
    ...
    case "debian":
      return `sudo apt install ./Ferax_${version}_amd64.deb`;
    case "fedora":
      return `sudo dnf install ./Ferax-${version}-1.x86_64.rpm`;
  }
}
          ...
          <DialogTitle>
            {ready
              ...
              : downloading
                ? "Downloading updateâ€¦"
                : manual
                  ? `Ferax v${manual.version} is available`
                  : `Ferax v${update?.version} is available`}
          </DialogTitle>
          <DialogDescription>
            {ready
              ? "Restart Ferax to finish installing."
              : downloading
                ? progress !== null
                  ? `${progress.toFixed(0)}% â€” ${formatBytes(status.downloaded)}`
