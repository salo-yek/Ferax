...
const SIDEBAR_WIDTH_STORAGE_KEY = "ferax.sidebar.width";
const SIDEBAR_VIEW_STORAGE_KEY = "ferax.sidebar.view";
      ...
      (event) => {
        ...
        void (async () => {
          try {
            ...
            if (!parsed.ok) {
              console.warn("[ferax] theme not applied:", parsed.error);
              return;
            }
            await saveCustomTheme(parsed.theme);
          } catch (e) {
            console.warn("[ferax] theme ingest failed:", e);
          }
        })();
      },
    );
  ...
  const handleAttachFileToAgent = useCallback(
    (path: string) => {
      ...
      window.dispatchEvent(
        new CustomEvent<string>("ferax:ai-attach-file", { detail: path }),
      );
      openPanel();
      focusInput(null);
    },
