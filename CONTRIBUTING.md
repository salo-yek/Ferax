# Contributing to Ferax

Ferax is a solo-maintained project with a strong product direction. Contributions are welcome, but **alignment matters more than volume**.
...
- Ferax has one active maintainer ([@crynta](https://github.com/crynta)).
...
Ferax positions itself as **lightweight, fast, production-grade**. Every PR is reviewed against:
...
The most common way a PR breaks Ferax is a **local fix with global blast radius**: the diff solves one reported case, reads fine, passes type-check and clippy, and silently breaks the same subsystem in every other case. Review alone does not catch these. A test does.
...
- Ferax is not trying to be a full IDE replacement (VS Code, Cursor, Zed).
