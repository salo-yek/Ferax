...
fn generate_sentinel() -> String {
    ...
    format!("__Ferax_CWD_{:016x}_{:016x}__", mix, counter)
}
...
fn wrap_with_sentinel(command: &str, workspace: &WorkspaceEnv, sentinel: &str) -> String {
    ...
    {
        format!(
        "{command}\n$__Ferax_rc = if ($null -ne $LASTEXITCODE) {{ $LASTEXITCODE }} elseif ($?) {{ 0 }} else {{ 1 }}\n\"`n{sentinel}$($PWD.Path)\"\nexit $__Ferax_rc\n",
    )
    }
}
...
mod tests {
    ...
    fn sentinels_are_unique_per_session() {
        ...
        assert!(a.sentinel.starts_with("__Ferax_CWD_"));
    ...
    fn strip_uses_session_sentinel_only() {
        ...
        let attacker = "__Ferax_CWD_0000000000000000_0000000000000000__/evil";
        let trailer = format!("\n{}/real\n", s.sentinel);
