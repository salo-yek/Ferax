...
const OWNED_MARKERS: [&str; 2] = ["notify;Ferax;", "ferax;notify"];

// Gated on FERAX_TERMINAL; no-op outside Ferax. Returns the sequence via
// `terminalSequence` because hooks lost /dev/tty access in v2.1.139.
fn hook_cmd(event: &str) -> String {
    format!(
        r#"[ -n "$FERAX_TERMINAL" ] && printf '{{"terminalSequence":"\\u001b]777;notify;Ferax;{event}\\u0007"}}' || true"#
    )
}
...
pub fn agent_enable_claude_hooks() -> Result<(), String> {
    ...
    let tmp = path.with_extension("json.ferax-tmp");
...
pub fn agent_claude_hooks_status() -> bool {
    ...
    HOOK_EVENTS
        .iter()
        .all(|(_, m)| content.contains(&format!("notify;Ferax;{m}")))
}
...
mod tests {
    ...
    fn adds_all_event_hooks_to_empty_config() {
        ...
        assert!(command(&out, "Notification", 0).contains("notify;Ferax;attention"));
        assert!(command(&out, "Stop", 0).contains("notify;Ferax;finished"));
        assert!(command(&out, "UserPromptSubmit", 0).contains("notify;Ferax;working"));
        ...
        let legacy = json!({
            "hooks": {
                "Notification": [
                    { "hooks": [ {
                        "type": "command",
                        "command": "[ -n \"$FERAX_TERMINAL\" ] && printf '\\033]777;ferax;notify\\033\\\\' > /dev/tty || true"
                    } ] }
                ]
            }
        });
