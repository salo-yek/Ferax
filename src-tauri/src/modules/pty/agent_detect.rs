...
const FERAX_MARKER: &[u8] = b"notify;Ferax;";
...
impl AgentDetector {
    ...
    fn handle_osc777<F: FnMut(Transition)>(&mut self, pt: &[u8], emit: &mut F) {
        if let Some(event) = pt.strip_prefix(FERAX_MARKER) {
...
mod tests {
    ...
    #[test]
    fn ferax_marker_drives_status() {
        let mut d = AgentDetector::new();
        run(&mut d, &osc("133;C;claude"));
        assert_eq!(run(&mut d, &osc("777;notify;Ferax;attention")), vec![Transition::Attention]);
        assert_eq!(run(&mut d, &osc("777;notify;Ferax;working")), vec![Transition::Working]);
        assert!(run(&mut d, &osc("777;notify;Ferax;working")).is_empty());
        assert_eq!(run(&mut d, &osc("777;notify;Ferax;finished")), vec![Transition::Finished]);
    }

    #[test]
    fn ferax_marker_auto_arms_without_preexec() {
        let mut d = AgentDetector::new();
        assert_eq!(
            run(&mut d, &osc("777;notify;Ferax;attention")),
